...
# Get word frequencies for individual books ----
...
hg_tf_idf <- hg_tf_idf %>%
  ...
  mutate(gutenberg_id = replace(gutenberg_id, 
                                gutenberg_id == '1013', 'The Invisible Man')) %>%
  mutate(gutenberg_id = replace(gutenberg_id, 
                                gutenberg_id == '5230', 'The Island of Dr. Moreau')) %>%
  rename(book = gutenberg_id)

bs_tf_idf <- bs_tf_idf %>%
  ungroup() %>%
  mutate(gutenberg_id = replace(gutenberg_id, 
                                gutenberg_id == '767', 'Dracula')) %>%
  mutate(gutenberg_id = replace(gutenberg_id, 
                                gutenberg_id == '768', 'Carmilla')) %>%
  mutate(gutenberg_id = replace(gutenberg_id, 
                                gutenberg_id == '969', 'Varney the Vampire')) %>%
  mutate(gutenberg_id = replace(gutenberg_id, 
                                gutenberg_id == '1260', 'The Beetle')) %>%
  mutate(gutenberg_id = replace(gutenberg_id, 
                                gutenberg_id == '9182', 'The Vampyre')) %>%
  rename(book = gutenberg_id)
